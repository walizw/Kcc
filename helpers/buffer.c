...
void
buffer_extend (struct buffer *buffer, size_t size)
{
...
void
buffer_need (struct buffer *buffer, size_t size)
{
...
void
buffer_printf (struct buffer *buffer, const char *fmt, ...)
{
  va_list args;
  va_start (args, fmt);
...
void
buffer_printf_no_terminator (struct buffer *buffer, const char *fmt, ...)
{
  va_list args;
  va_start (args, fmt);
  int index = buffer->len;
...
void
buffer_write (struct buffer *buffer, char c)
{
...
void *
buffer_ptr (struct buffer *buffer)
{
  return buffer->data;
}

char
buffer_read (struct buffer *buffer)
{
...
char
buffer_peek (struct buffer *buffer)
{
  if (buffer->rindex >= buffer->len)
    {
      return -1;

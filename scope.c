/*
 * scope.c - Manages scopes (of variables, function arguments, bodies, etc).
 *
...
void
scope_dealloc (struct scope *scope)
{
  // TODO: This
}

struct scope *
scope_create_root (struct compile_process *process)
{
...
void
scope_free_root (struct compile_process *process)
{
...
void
scope_iteration_start (struct scope *scope)
{
...
void
scope_iteration_end (struct scope *scope)
{
  // Does nothing
}

void *
scope_iterate_back (struct scope *scope)
{
  if (vector_count (scope->entities) == 0)
    return NULL; // nothing here :p

  return vector_peek_ptr (scope->entities);
}

void *
scope_last_entity_at_scope (struct scope *scope)
{
  if (vector_count (scope->entities) == 0)
    return NULL; // nothing here :p
...
void *
scope_last_entity_from_scope_stop_at (struct scope *scope,
...
void *
scope_last_entity_stop_at (struct compile_process *process,
                           struct scope *stop_scope)
{
...
void *
scope_last_entity (struct compile_process *process)
{
  return scope_last_entity_stop_at (process, NULL);
}

void
scope_push (struct compile_process *process, void *ptr, size_t elm_size)
{
  vector_push (process->scope.current->entities, &ptr);
  process->scope.current->size += elm_size;
}

void
scope_finish (struct compile_process *process)
{
...
struct scope *
scope_current (struct compile_process *process)
{
